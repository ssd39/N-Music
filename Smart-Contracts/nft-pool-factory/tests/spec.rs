...
const nft_WHITELIST_ACCOUNT_ID: &str = "staking-pool-whitelist";
const nft_ID: &str = "pool";
const nft_ACCOUNT_ID: &str = "pool.factory";
const OWNER_STAKING_ACCOUNT_ID: &str = "owner-staking";

#[test]
fn create_nft_success() {
    let (mut r, foundation, owner) = setup_factory();

    let res: U128 = view_factory(&r, "get_min_attached_balance", "");
    assert_eq!(res.0, ntoy(30));

    let res: u64 = view_factory(&r, "get_number_of_nfts_created", "");
    assert_eq!(res, 0);
    ...
    let is_whitelisted: bool = call_view(
        ...
        &serde_json::to_string(
            &json!({ "nft_account_id": nft_ACCOUNT_ID.to_string() }),
        )
        .unwrap(),
    );
    ...
    let pool_account = r.view_account(&nft_ACCOUNT_ID.to_string());
    ...
    let actual_staking_key: Base58PublicKey =
        call_view(&r, &nft_ACCOUNT_ID, "get_staking_key", "");
    assert_eq!(actual_staking_key.0, staking_key.0);
}

#[test]
fn create_nft_bad_staking_key() {
    let (mut r, foundation, owner) = setup_factory();
...
fn setup_factory() -> (RuntimeStandalone, ExternalUser, ExternalUser) {
    ...
    foundation
        .init_whitelist(&mut r, nft_WHITELIST_ACCOUNT_ID.to_string())
        .unwrap();
    ...
    foundation
        .init_factory(&mut r, &nft_WHITELIST_ACCOUNT_ID)
        .unwrap();
    (r, foundation, owner)
}
