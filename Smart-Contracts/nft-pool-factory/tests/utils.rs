...
lazy_static::lazy_static! {
    static ref FACTORY_WASM_BYTES: &'static [u8] = include_bytes!("../res/nft_factory.wasm").as_ref();
...
impl ExternalUser {
    ...
    pub fn init_factory(
        ...
        let tx = self
            ...
            .function_call(
                "new".into(),
                serde_json::to_vec(&json!({"nft_whitelist_account_id": nft_whitelist_account_id.to_string()})).unwrap(),
                MAX_GAS,
                0,
            )
            .sign(&self.signer);
    ...
    pub fn init_whitelist(
        ...
        let tx = self
            .new_tx(runtime, nft_whitelist_account_id)
            .create_account()
            .transfer(ntoy(30))
